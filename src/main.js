...
function readConfig() {
  ...
  const valMin = Math.max(0, num('cfg-val-min', 1));
  const valMax = Math.max(valMin, num('cfg-val-max', 9));
  ...
  const target = num('cfg-target', RULE_REGISTRY[ruleId]?.defaultTarget ?? 10);
